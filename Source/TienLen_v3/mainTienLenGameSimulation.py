#main TienLenPPOSimulation class

import numpy as np
from ppo.PPONetwork import PPONetwork, PPOModel
from logic.TienLenGame import vectorizedTienLenGame
...
class TienLenPPOSimulation(object):
    
    def __init__(
        self, sess, *, 
        inpDim = cons.SIZE_OBSERVATIONS,
         nGames = 8, 
         nSteps = 20, 
         nMiniBatches = 4, 
         nOptEpochs = 5, 
        ...
        self.trainingNetwork = PPONetwork(sess, inpDim, cons.SIZE_ACTION_SPACES, "trainNet")
        self.trainingModel = PPOModel(sess, self.trainingNetwork, inpDim, cons.SIZE_ACTION_SPACES, ent_coef, vf_coef, max_grad_norm)
        ...
        #environment
        self.vectorizedGame = vectorizedTienLenGame(nGames)
    ...
    def run(self):
        ...
        for _ in range(self.nSteps):
            ...
            print("I get actions {} ".format(actions))
...
if __name__ == "__main__":
    ...
    with tf.Session() as sess:
        mainSim = TienLenPPOSimulation(sess, nGames=64, nSteps=20, learningRate = 0.00025, clipRange = 0.2)
        start = time.time()
