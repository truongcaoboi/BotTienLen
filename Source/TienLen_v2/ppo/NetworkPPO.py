...
class ActionNetwork(nn.Module):
    ...
    def forward(self, state, actionAva, is_get_action):
        print(state.size())
        if(is_get_action):
            self.time_get_action += 1
        dist = self.actor(state)
        print("In times {} dist when forward: {} ".format(self.time_get_action, dist))
        mask = T.tensor(actionAva, dtype=T.bool).to(self.device)
        # mask = mask.reshape((1, len(actionAva)))
        dist[~mask] = -math.inf
        dist = T.softmax(dist, dim=0)
...
class Agent:
    def __init__(self,input_dims, n_actions, gamma = 0.99, alpha = 0.0003, gae_lambda = 0.95,
                 policy_clip = 0.1, batch_size = 64, N = 2024, n_epochs = 1, memory=None):
        self.gamma = gamma
        self.alpha = alpha
    ...
    def choose_action(self, observation, actionAva):
        ...
        action_ex = action.item()
        if(actionAva[action_ex] == 1):
            print("chon dung action")
        else:
            print("chon sai action")
        # input()
        print("index action {}".format(action_ex))
